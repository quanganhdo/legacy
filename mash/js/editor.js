	...
	function prepareEditor() {
		...
		$('#generate').click(function() {
			var output = '';
            $.each(customCSS, function(i, n) {
                output += i + ' {\n';
                $.each(n, function(j, nn) {
        ...
        $('#get').click(function() {
            $('.step2, .tips').hide();
            $('.step1 .styleBox li').removeClass('selected').addClass('unselected');
            $('#stealCSS .step2, #stealCSS .tips').fadeIn();
        });
        ...
        $('#steal').click(function() {
            $.get('getcss.php', {l:$('#360URL')[0].value}, function(src){
                if (src == '500') {
                    $('#stolenCSS')[0].value = 'Lỗi: URL sai quy cách';
                    return;
                } else {
                    var crzCSS = 'body {';
                    // body first
                    crzCSS += '\n\tbackground:' + src.match(/#doc-body {[\s\S]*?background:(.*)[\s\S]*?padding-top:(.*)/)[1];
                    crzCSS += '\n\tpadding-top:' + src.match(/#doc-body {[\s\S]*?background:(.*)[\s\S]*?padding-top:(.*)/)[2];
                    // txt
                    crzCSS += '\n\tcolor:' + src.match(/#doc-body .intro .footnote {[\s\S]*?color:(.*)[\s\S]*?font-family:(.*)/)[1];
                    crzCSS += '\n\tfont-family:' + src.match(/#doc-body .intro .footnote {[\s\S]*?color:(.*)[\s\S]*?font-family:(.*)/)[2];
                    crzCSS += '\n}';
                    // module
                    crzCSS += '\n.mod {';
                    crzCSS += '\n\tbackground-color:' + src.match(/#doc-body #ymgl-blog dd.post-body .poll dd {[\s\S]*?background-color:(.*)/)[1];
                    crzCSS += '\n\tborder-color:' + src.match(/#doc-body .top,[\s\S]*?#doc-body .bottom {[\s\S]*?border-color:(.*)/)[1];
                    crzCSS += '\n\tborder-style:solid;';
                    crzCSS += '\n}';
                    // h3
                    crzCSS += '\nh3 {';
                    crzCSS += '\n\tfont-family:' + src.match(/#doc-body #ymgl-blog .comments .head h4 {[\s\S]*?font-family:(.*)[\s\S]*?color:(.*)/)[1];
                    crzCSS += '\n\tcolor:' + src.match(/#doc-body #ymgl-blog .comments .head h4 {[\s\S]*?font-family:(.*)[\s\S]*?color:(.*)/)[2];
                    crzCSS += '\n}';
                    // link
                    crzCSS += '\na {';
                    crzCSS += '\n\tcolor:' + src.match(/#doc-body #ymgl-blast p a:visited {[\s\S]*?color:(.*)/)[1];
                    crzCSS += '\n}';
                    crzCSS += '\na:visited {';
                    crzCSS += '\n\tcolor:' + src.match(/#doc-body table a:visited {[\s\S]*?color:(.*)/)[1];
                    crzCSS += '\n}';
                    $('#stolenCSS')[0].value = crzCSS;
                    // auto apply
                    t = new Object();
                    u = $('#stolenCSS')[0].value;
                    c = $('#stolenCSS')[0].value.match(/(.*?){/g);
	...
	$(document).ready(function() {
		resizeFrame();
		prepareEditor();
        $('#about')[0].innerHTML = 'Version 0.4 BETA &bull; Coded by QAD &bull; Inspired by Real Editor';
	});
